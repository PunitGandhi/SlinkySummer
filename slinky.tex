...
discuss the benifits of this curriculum as a vehicle for learning about model building in \sec{discussion}.
...
In developing this model, students face many choices regarding approximations and simplifications. Which choices that render the mathematics more tractable, such as choosing to model only a few masses, yield a system that reproduces ``levitation'' phenomenon? What roles do the various parameters of the model---number, mass, spring constant---play in reproducing the relevant qualities of a slinky? To answer these questions, and to glean insight into the slinky drop, the students explore the model in two complementary ways: In section \ref{subsec:forcesexperiment} they build a physical approximation of the discrete system in order to experimentally explore the model, while in section \ref{subsec:forcesnumeric} they numerically evolve the derived equations of motion.  

%The experimental approach has the freedom to explore the model  whereas the numerical approach avoids the engineering constraints and exerimental uncertainty.
